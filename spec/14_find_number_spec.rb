...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      answer = 8
      guess = 8
      let(:number_guessing) { double('random_number', value: answer) }
      subject(:game_over) { described_class.new(0, 9, number_guessing, guess) }
    ...
    context 'when guess and random_number are not equal' do
      ...
      guess = 4
      let(:number_guessing) { double('random_number', value: answer) }
      subject(:game_not_over) { described_class.new(0, 9, number_guessing, guess) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'does not update max' do
        expect { low_guess_game.update_range }.to_not change { low_guess_game.max }.from(9)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        expect { high_guess_game.update_range }.to_not change { high_guess_game.min }.from(0)
      end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      subject(:one_option_game) { described_class.new(5, 8, random_number, 7) }

      it 'updates min to the same value as max' do
        expect { one_option_game.update_range }.to change { one_option_game.min }.from(5).to(8)
      end

      it 'does not update max' do
        expect { one_option_game.update_range }.to_not change { one_option_game.max }.from(8)
      end
    end
  end
end
